function params = param_struct();
    ...
    params.ts_acq = 50e-06; %%% acquisition sampling time - this defines the new sampling when computing the tf and saved on the HD
    ...
    %% CONTROL SENSITIVITY
    %%% MIC    (SWAPPED!!!)
    params.sens_p_m =  -1/37.571508E-3;% 1/(V/Pa) SN65606 %%%%%%%%%%%%%%%%%%%%%
    %params.sens_p_c =  -1/20.00E-3;% 1/(V/Pa) SN6969 NOT IN USE!
    params.sens_p_p =  -1/38.909796E-3;% 1/(V/Pa) SN65608
    ...
    params.pb2disp_m = 2.055749e-05 ;  %Trasnferfunction between backpressure and displacement (m/V) *SN 65606
    params.pb2disp_p = 1.95721e-05;   %Trasnferfunction between backpressure and displacement (m/V) *SN 65608
    ...
    %%% speaker 1: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % %Re_m = 7.43;
    params.Bl_m  =  1.587001e+00;
    params.Rms_m =  3.548351e-01;
    params.Mms_m =  7.226418e-04;
    params.Cmc_m =  2.145745e-04;%
    
    params.f0_m = 1/(2*pi*sqrt(params.Mms_m*params.Cmc_m)); % Resonnance frequency (Hz)
    
    %%% speaker 2: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % % Re_p =  3.57;
    params.Bl_p  =  1.507391e+00;%1.3803;
    params.Rms_p =  2.814832e-01;% 0.3005;
    params.Mms_p =  6.628828e-04;%6.1805e-04;
    params.Cmc_p =  2.307496e-04;%1.705e-04;%1.7219e-04;%
    ...
    params.kappa = -1e-3; %0*(params.muC_m/params.Cmc_m);%(params.muC_m/params.Cmc_m); % 20221103 update - the control was inverted... 
    params.rho = -0e3; %0*params.muR_m*params.Rms_m;%(params.muR_m*params.Rms_m); % local non-linearity
end
