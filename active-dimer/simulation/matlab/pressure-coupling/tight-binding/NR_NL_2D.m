...
psi_0 = zeros(M, N);
psi_0(floor(M/4),floor(3*N/4)) = 1;%dirac
...
%%% dynamics
[t_raw,psi_raw] = ode45(@(t, psi) nl_nh_2D(t,psi,H), tspan, psi_0); 



psi2D = reshape(psi_raw, numel(tspan), N, M);%
...
%% FUNCTIONS
function dpdt = nl_nh_2D(t, p, H) % (time, Hamiltonian, psi)
%%% ADD BC HERE
dpdt =  -1i*H*p; 
end
