function sys_param = sys_params();
    ...
    sys_param.A_src = 10; %%% incident pressure amplitude (Pa) %%% 
    ...
    sys_param.src_loc = [80];%source location (at each circuit node)
  ...
  % coupling kappa>0 => v>w; kappa <0 => v>w; 
    ...
    gamma_v_A = +0.5; % v asymmetry --> Coupling to left if >0
    gamma_w_A = +0.5;  % w asymmetry
    gamma_v_B = -0.5; % v asymmetry  
    gamma_w_B = -0.5; % w asymmetry 
