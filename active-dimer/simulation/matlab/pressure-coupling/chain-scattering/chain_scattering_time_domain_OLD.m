%%% SCATTERING THROUGH AN ACOUSTIC CHAIN OF SCATTERERS IN TIME DOMAIN %%%%%%%%%%%%%%%%%%
clear all
close all
clc
...
for f = fi:Df:ff %Hz
    ...
    subplot(2,2,1);
    %hold on
    plot(t_out*1000, y_out(:,3)*1000,'k') 
...
function dydt = odefun(t,y,f)
    ...
    dydt(5) = vtra_1; %vt4
    dydt(6) = (2/(Ma+Ma_btwn))*(gamma1 - 1/Ca_btwn*(xtra_1-xtot_2));%3
    
    %%%  Atom 2
    dydt(7) = vtot_2; 
    dydt(8) = (2/(Ma+Ma_btwn))*(-gamma2 +  1/Ca_btwn*(xtra_1-xtot_2)) ;%5
    dydt(9) = vspk_2; 
