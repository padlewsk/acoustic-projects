...
k = 2*pi*f/c0; %+ 70%corresponding wave vector
%N = length(k);
...
l1 = x2 - offset; %distance between left edge of specimen and mic 2 (c.f. 20220629 notes)
l2 = x3 - offset ; %distance between left edge of specimen and mic 3
s1 = abs(x2-x1); %mic separation
s2 = abs(x4-x3);%mic separation

%% SYMMETRIC CASE
%{ 
%%OLD
load('./smatrix_data/H21_corr.mat');
load('./smatrix_data/H31_corr.mat');
load('./smatrix_data/H41_corr.mat');

H11_a = ones(N,1);
H21_a = load('./smatrix_data/H21_a.mat').H21_a.*H21_corr;
H31_a = load('./smatrix_data/H31_a.mat').H31_a.*H31_corr;
H41_a = load('./smatrix_data/H41_a.mat').H41_a.*H41_corr;
...
s11 = (A.*B-C.*D)./(A.^2-D.^2.*exp(2*1i*k*a));
s12 = (A.*C.*exp(-1i*k*a) - B.*D.*exp(1i*k*a))./(A.^2-D.^2.*exp(2*1i*k*a));
s22 = s11;
s21 = s12;
...
H11_b = ones(N,1);
H21_b = load('./smatrix_data/H21_b.mat').H21_b.*H21_corr;
H31_b = load('./smatrix_data/H31_b.mat').H31_b.*H31_corr;
H41_b = load('./smatrix_data/H41_b.mat').H41_b.*H41_corr;
...
figure(1);
hold on
plot(abs(q_real)*a/pi,f,'-r','LineWidth',3)
plot(abs(q_imag)*a/pi,f,'-k','LineWidth',3)
plot(k*a/pi,f,'-b','LineWidth',1)
plot(2-k*a/pi,f,'-b','LineWidth',1)
