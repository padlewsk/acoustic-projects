...
X_omega_0 = (1/T)*trapz(t,real(exp(-1i*p.omega_0*t)).*psi_t);
Y_omega_0 = (1/T)*trapz(t,real(exp(-1i*(p.omega_0*t + pi/2))).*psi_t);
R_omega_0 = 2*sqrt(X_omega_0^2 + Y_omega_0^2); %%%!!! 2X?
phi_omega_0 = atan2(Y_omega_0,X_omega_0); %rad

%
X_omega_1 = (1/T)*trapz(t,real(exp(-1i*(p.omega_1*t))).*psi_t);
Y_omega_1 = (1/T)*trapz(t,real(exp(-1i*(p.omega_1*t + pi/2))).*psi_t);
...
phi = mod(-(phi_omega_1 - p.omega_1/p.omega_0*phi_omega_0)- pi/2,2*pi);
...
plot(t,real(R_omega_0*exp(-1i*(p.omega_0*t + phi_omega_0)) + R_omega_1*exp(-1i*(p.omega_1*t + phi_omega_1))),'--','LineWidth',2)
