...
%time scale <-------------------------------------------------------
delta_t = 0.001*(2*pi)/omega_1_q; %avoid integer multiples --> non separable solutions?
%delta_t = (2*pi)/omega_0; 
...
%
a_00_est =mean(a_1_t_est.*a_2_t_est) %1/tspan(end)*trapz(t,a_1_t_est.*a_2_t_est)
a_01_est =mean(a_1_t_est.*b_2_t_est) %1/tspan(end)*trapz(t,a_1_t_est.*b_2_t_est)
a_10_est =mean(b_1_t_est.*a_2_t_est) %1/tspan(end)*trapz(t,b_1_t_est.*a_2_t_est)
a_11_est =mean(b_1_t_est.*b_2_t_est) %1/tspan(end)*trapz(t,b_1_t_est.*b_2_t_est)
%}
